...
uint8_t DtaDevOpal::initialSetup(char * password, bool securemode)
{
    ...
    if ((lastRC = activateKpioSP((securemode)?(char*)newpwd->c_str():password)) != 0) {
		LOG(E) << "Initial setup failed - unable to activate kpioSP";
		return lastRC;
	}

    if ((lastRC = setKpioKeyTagAllocation(1, 1, (securemode)?(char*)newpwd->c_str():password)) != 0) {
		LOG(E) << "Initial setup failed - unable to set kpioKeyTagAllocation";
		return lastRC;
	}

    if ((lastRC = setKpioPolicies(0x0b, (securemode)?(char*)newpwd->c_str():password)) != 0) {
		LOG(E) << "Initial setup failed - unable to set kpioPolicies";
		return lastRC;
	}
...
uint8_t DtaDevOpal::exec(DtaCommand * cmd, DtaResponse & resp, uint8_t protocol, uint16_t com_id)
{
    ...
    LOG(E) << endl << "Dumping command buffer";
    IFLOG(E) DtaHexDump(cmd->getCmdBuffer(), SWAP32(hdr->cp.length) + sizeof (OPALComPacket));
...
uint8_t DtaDevOpal::activateKpioSP(char * password)
{
    LOG(D1) << "Entering DtaDevOpal::activateKpioSP()";
	uint8_t lastRC;
	vector<uint8_t> table;
	table. push_back(OPAL_SHORT_ATOM::BYTESTRING8);
	for (int i = 0; i < 8; i++) {
		table.push_back(OPALUID[OPAL_UID::OPAL_KPIOSP_UID][i]);
	}
	DtaCommand *cmd = new DtaCommand();
	if (NULL == cmd) {
		LOG(E) << "Unable to create command object ";
	...
	if ((lastRC = getTable(table, 0x06, 0x06)) != 0) {
		LOG(E) << "Unable to determine KpioSP Lifecycle state";
		delete cmd;
		delete session;
		return lastRC;
	}
	if ((0x06 != response.getUint8(3)) || // getlifecycle
		(0x08 != response.getUint8(4))) // Manufactured-Inactive
	{
		LOG(E) << "Kpio SP lifecycle is not Manufactured-Inactive";
		delete cmd;
		delete session;
		return DTAERROR_INVALID_LIFECYCLE;
	}
	cmd->reset(OPAL_UID::OPAL_KPIOSP_UID, OPAL_METHOD::ACTIVATE);
	cmd->addToken(OPAL_TOKEN::STARTLIST);
	cmd->addToken(OPAL_TOKEN::ENDLIST);
	cmd->complete();
	if ((lastRC = session->sendCommand(cmd, response)) != 0) {
	...
	LOG(I) << "KPIO SP Activate Complete";

	delete cmd;
	delete session;
	LOG(D1) << "Exiting DtaDevOpal::activateKpioSP()";
	return 0;
}

uint8_t DtaDevOpal::setKpioKeyTagAllocation(uint32_t ns_id, uint8_t managed, char * password)
{
	uint8_t lastRC;
	LOG(D1) << "Entering DtaDevOpal::setKpioKeyTagAllocation()";
	vector<uint8_t> KT;
	KT.push_back(OPAL_SHORT_ATOM::BYTESTRING8);
	for (int i = 0; i < 8; i++) {
		KT.push_back(OPALUID[OPAL_UID::KPIO_KEYTAG_ALLOCATION_TABLE][i]);
	}
    KT[8] = ns_id;
	session = new DtaSession(this);
	if (NULL == session) {
	...
	if ((lastRC = session->start(OPAL_UID::OPAL_KPIOSP_UID, password, OPAL_UID::OPAL_ADMIN1_UID)) != 0) {
        LOG(E) << "Start Session Faild";
		delete session;
		return lastRC;
	}
	DtaCommand *set = new DtaCommand();
	...
	set->addToken(OPAL_TOKEN::STARTNAME);
	set->addToken(0x4);
	set->addToken(managed ? OPAL_TRUE : OPAL_FALSE);
	set->addToken(OPAL_TOKEN::ENDNAME);
	set->addToken(OPAL_TOKEN::ENDLIST);
	set->addToken(OPAL_TOKEN::ENDNAME);
	set->addToken(OPAL_TOKEN::ENDLIST);
	set->complete();
	if ((lastRC = session->sendCommand(set, response)) != 0) {
		LOG(E) << "setKpioKeyTagAllocation Managed Failed ";
		delete set;
		delete session;
		return lastRC;
	}

    LOG(I) << "KPIO SP set KpioKeyTagAllocation Managed " << (managed ? "True" : "False");
    
	delete set;
	delete session;
	LOG(D1) << "Exiting DtaDevOpal::setKpioKeyTagAllocation()";
	return 0;
}

uint8_t DtaDevOpal::setKpioPolicies(uint8_t policies_mask, char * password)
{
    uint8_t lastRC;
	LOG(D1) << "Entering DtaDevOpal::setKpioPolicies()";
	vector<uint8_t> PL;
	PL.push_back(OPAL_SHORT_ATOM::BYTESTRING8);
	for (int i = 0; i < 8; i++) {
		PL.push_back(OPALUID[OPAL_UID::KPIO_POLICIES_TABLE][i]);
	}
	session = new DtaSession(this);
	if (NULL == session) {
		LOG(E) << "Unable to create session object ";
	...
	set->reset(OPAL_UID::OPAL_AUTHORITY_TABLE, OPAL_METHOD::SET);
	set->changeInvokingUid(PL);
	set->addToken(OPAL_TOKEN::STARTLIST);
	set->addToken(OPAL_TOKEN::STARTNAME);
	set->addToken(OPAL_TOKEN::VALUES);
	set->addToken(OPAL_TOKEN::STARTLIST);
    for (int i=0; i<7; i++) {
        if (policies_mask & (1 << i)) {
            set->addToken(OPAL_TOKEN::STARTNAME);
            set->addToken(i+1);
            set->addToken(OPAL_TRUE);
            set->addToken(OPAL_TOKEN::ENDNAME);
        }
    }
	set->addToken(OPAL_TOKEN::ENDLIST);
	...
	set->complete();
	if ((lastRC = session->sendCommand(set, response)) != 0) {
		LOG(E) << "setKpioPolicies Failed ";
		delete set;
		delete session;
		return lastRC;
	}
    
    LOG(I) << "KPIO SP set setKpioPolicies Mask " << policies_mask;
    
	delete set;
	delete session;
	LOG(D1) << "Exiting DtaDevOpal::setKpioPolicies()";
	return 0;
}
...
uint8_t DtaDevOpal::sendKmipCommand(char * password, char * filename)
{
    ...
    cmd->bufferpos = 512;
    uint32_t size = cmd->bufferpos - sizeof(OPALComPacket);
    OPALComPacket * hdr = (OPALComPacket *)cmd->cmdbuf;
    hdr->length = SWAP32(size);
    ifstream kmipfile;
    kmipfile.open(filename, ios::in | ios::binary);
    kmipfile.read((char*)cmd->cmdbuf + sizeof(OPALComPacket), size);
    uint8_t exec_rc = exec(cmd, response, 0x3, comID()+0x2);
    if (exec_rc != 0) {
