...
public:
    ...
    uint8_t start(OPAL_UID SP, uint16_t com_id=0xffff, uint8_t protocol_id=0xff);    
	...
	* @param SignAuthority the Signing authority (in a simple session this is the user)
	*/
    uint8_t start(OPAL_UID SP, char * HostChallenge, OPAL_UID SignAuthority, uint16_t com_id=0xffff, uint8_t protocol_id=0xff);
    /** Start an authenticated session (OPAL only) 
     * @param SP the securitly provider to start the session with
     * @param HostChallenge the password to start the session
     * @param SignAuthority the Signing authority (in a simple session this is the user)
     *  */
    uint8_t start(OPAL_UID SP, char * HostChallenge, vector<uint8_t> SignAuthority, uint16_t com_id=0xffff, uint8_t protocol_id=0xff);
     ...
     */
    uint8_t sendCommand(DtaCommand * cmd, DtaResponse & response, uint16_t com_id=0xffff, uint8_t protocol_id=0xff);
private:
