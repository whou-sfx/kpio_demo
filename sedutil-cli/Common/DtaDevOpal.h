...
protected:
    ...
    /** Ask the user to input a new password. 
      ...
      */
#ifdef __linux__
    uint8_t askNewPassword(std::shared_ptr<SecureString> &password, bool confirm = false);
#endif //__linux__

    uint8_t activateKpioSP(char * password);

    uint8_t setKpioKeyTagAllocation(uint32_t ns_id, uint8_t managed, char * password);

    uint8_t setKpioPolicies(uint8_t policies_mask, char * password);
};
