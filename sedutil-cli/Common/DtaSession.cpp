...
uint8_t
DtaSession::start(OPAL_UID SP, uint16_t com_id, uint8_t protocol_id)
{
    return (start(SP, NULL, OPAL_UID::OPAL_UID_HEXFF, com_id, protocol_id));
}

uint8_t 
DtaSession::start(OPAL_UID SP, char * HostChallenge, OPAL_UID SignAuthority, uint16_t com_id, uint8_t protocol_id)
{
	...
	return(start(SP, HostChallenge, auth, com_id, protocol_id));
}
...
#ifdef MULTISTART
uint8_t
DtaSession::start(OPAL_UID SP, char * HostChallenge, vector<uint8_t> SignAuthority, uint16_t com_id, uint8_t protocol_id)
{
	vector <uint8_t> auth;
...
#endif
{
    ...
    cmd->complete();
	if ((lastRC = sendCommand(cmd, response, com_id, protocol_id)) != 0) {
...
uint8_t
DtaSession::sendCommand(DtaCommand * cmd, DtaResponse & response, uint16_t com_id, uint8_t protocol_id)
{
    ...
    uint8_t exec_rc = 0;
    if (protocol_id == 0xff) {
        exec_rc = d->exec(cmd, response, SecurityProtocol, com_id);
    } else {
        exec_rc = d->exec(cmd, response, protocol_id);
    }
    if (0 != exec_rc)
    {
